#!/usr/bin/env python3
"""
Stress test suite for Thesidia forensic pipeline.
...
def run_stress_test():
    ...
    print("=" * 80)
    print("THESIDIA FORENSIC PIPELINE - STRESS TEST SUITE")
    print("=" * 80)
    ...
    for test in STRESS_TEST_QUERIES:
        ...
        try:
            ...
            if isinstance(result_raw, dict):
                result = result_raw.get('response', str(result_raw))
            else:
                result = result_raw
            ...
            print(f"✓ Sections found: {len(sections_found)}/4")
    ...
    for r in results:
        if 'error' in r:
            ...
            print(f"Test {r['id']}: ✓ {r['elapsed']:.1f}s | {r['length']:,} chars | {r['sections']}/4 sections | {r['file']}")
