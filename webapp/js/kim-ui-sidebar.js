...
(function () {
    ...
    function addMessageToFeed(text, isSelf, senderName, messageId = null, readReceipts = null, reactions = null, parentMessageId = null, edited = false) {
        ...
        ${replyIndicator}
        <div class="kim-message-bubble">${window.ForensicUI ? window.ForensicUI.format(text, messageId) : escapeHtml(text)}</div>
        ${reactionsHtml}
    ...
    // Listen for Forensic Thread Clicks
    document.addEventListener('forensic-thread-click', async (e) => {
        const { query, parentId } = e.detail;

        // Show user query in chat immediately (as self)
        if (typeof addMessageToFeedHelper === 'function') {
            const tempId = 'req_' + Date.now();
            addMessageToFeedHelper(query, true, 'You', tempId);
        }

        try {
            // Show loading state or optimized update? 
            // For now, simple API call
            const response = await fetch('/api/forensic/query', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    // Add auth headers if needed
                    'X-User-ID': window.myUserId || localStorage.getItem('thesidia_user_id')
                },
                body: JSON.stringify({
                    query,
                    parent_id: parentId
            ...
            if (data.output && typeof addMessageToFeedHelper === 'function') {
                addMessageToFeedHelper(data.output, false, 'Thesidia', data.query_id);
            }

        } catch (error) {
            console.error('Error executing forensic thread:', error);
            // Show error in chat
            if (typeof addMessageToFeedHelper === 'function') {
                addMessageToFeedHelper('Error executing forensic query.', false, 'System');
            }
        }
    });
